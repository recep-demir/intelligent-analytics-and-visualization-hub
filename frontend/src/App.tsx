...
// Shared vertical legend pill used by every chart
...
export default function App() {
  ...
  const handleSearch = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (userRole === "Restricted" && sensitiveKeywords.some(w => query.toLowerCase().includes(w))) {
  ...
  function renderPieOrDonut(records: any[], isDonut: boolean) {
    ...
    const GAP = 0.9; // transparent gap between segments (% of full circle)
          ...
          <div className="flex gap-1 items-stretch">
            ...
            <div className="flex-1 relative h-72 bg-gray-950/40 rounded-xl border border-gray-800/80 p-4 overflow-hidden">
              ...
              <svg className="w-full h-full" viewBox={`0 0 ${VB_W} ${VB_H}`} preserveAspectRatio="none">
                ...
                {/* Invisible hover strips for tooltips */}
                {coords.map((c, i) => (
      ...
      <div className="w-full h-full flex items-center">
        <svg viewBox={`0 0 ${VW} ${VH}`} className="w-full" style={{ height: "380px" }}>
          {rects.map((r, i) => {
            ...
            // Always show value; only show name when box is large enough
            const showValue = rw > 14 && rh > 14;
            const showLabel = rw > 48 && rh > (showValue ? 38 : 22);
            const nameFontSize = Math.max(
              Math.min(rw / (r.item.name.length * 0.58), rh / 3.5, 15), 7
            );
            // Scale by character count so "CA$27.1K" always fits in its box
            const valFontSize = Math.max(
              Math.min(rw / (valStr.length * 0.62), rh / 2.2, 36), 8
            );
            const clipId = `clip-${i}`;
            return (
              <g key={i}>
  ...
  function renderHeatmap(records: any[]) {
    ...
    // Detect first-dimension key dynamically (province / category / status / productGroup)
                  ...
                  >
                    {(() => {
                      ...
                      const stops = records
                        .map((r: any, i: number) => {
                          const pct =
                            ((r.percentage ?? r.value ?? 0) / total) * 100;
                      ...
                      const getLabel = (r: any, i: number) =>
                        r[chartData.chartConfig.groupBy ?? ""] ??
                        r.province ??
                        r.status ??
                        r.label ??
                        r.name ??
                            ...
                            {records.map((record: any, i: number) => (
                              ...
                              >
                                <div className="flex items-center gap-2.5">
                                  <span
                                    className="w-2.5 h-2.5 rounded-md shadow-sm shrink-0"
                                    style={{
                                      backgroundColor:
                  ...
                  <div key="line-chart-view" className="w-full pt-4">
                    {(() => {
                      ...
                      if (points.length === 1) {
                        ...
                        const year =
                          points[0].year ?? points[0].tax_year ?? "—";
                        return (
                      ...
                      const getValue = (d: any) => {
                        const val =
                          d.amount ?? d.value ?? d.percentage ?? d.total;
